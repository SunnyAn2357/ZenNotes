...
function showToast(msg, undoData = null) {
  ...
  if (undoData) {
    ...
    undoBtn.onclick = () => {
      ...
      undoBtn.style.pointerEvents = "none"; // 🎯 취소 누르는 즉시 버튼 센서도 파괴!
    };

    // 0.5초 뒤에 버튼만 스르륵 나타나면서 터치 활성화
    setTimeout(() => {
      if (undoBtn) {
